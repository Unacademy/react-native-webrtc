...
@implementation RTCUNShader {
    ...
    BOOL _hasCurrentRotation;
    RTCVideoRotation _storedRotation;
...
//        RTCLog(@"Failed to setup vertex buffer");
    ...
    if (!_hasCurrentRotation || rotation != _storedRotation) {
        _hasCurrentRotation = YES;
        _storedRotation = rotation;
        RTCSetVertexData(_storedRotation);
    }
    return YES;
}
...
//        RTCLog(@"Failed to setup I420 program");
    ...
    glActiveTexture((GLenum)(GL_TEXTURE0 + kYTextureUnit));
    glBindTexture(GL_TEXTURE_2D, yPlane);
    
    glActiveTexture((GLenum)(GL_TEXTURE0 + kUTextureUnit));
    glBindTexture(GL_TEXTURE_2D, uPlane);
    
    glActiveTexture((GLenum)(GL_TEXTURE0 + kVTextureUnit));
...
//        RTCLog(@"Failed to setup NV12 shader");
    ...
    glActiveTexture((GLenum)(GL_TEXTURE0 + kUvTextureUnit));
