...
public class GlDrawer implements RendererCommon.GlDrawer {
    ...
    private static final String OES_FRAGMENT_SHADER_STRING =
            "#extension GL_OES_EGL_image_external : require\n"
                    ...
                    + "  vec4 color1 = vec4(color.r+1.5958*color.b, color.r-0.39173*color.g-0.81290*color.b, color.r+2.017*color.g, color.a);\n"
                    + "  vec4 hlv = rgb_to_hsv(color1);\n"
                    + "  float red = 0.0f;\n"
                    + "  float green = 1.0f;\n"
                    + "  float blue = 0.0f;\n"
                    + "  float accuracy = 0.50f;\n"
                    + "float d = 0.0f;\n"
                    + "if((2.0 - hlv.y)*hlv.z < 1.0f) {\n"
                    + " d = hlv.y*hlv.z /((2.0 - hlv.y)*hlv.z );\n"
                    + "}\n"
                    + "else {\n"
                    + "	d =  hlv.y*hlv.z/(2.0-(2.0-hlv.y)*hlv.z);\n"
                    + "}\n"
                    + "if (color.y >= 0.60 && color.x <= 0.4 && color.z <= 0.4) {\n"
                    + "              gl_FragColor = vec4(color.r, color.g, color.b, 0.0f);\n"
                    + "}else {\n"
